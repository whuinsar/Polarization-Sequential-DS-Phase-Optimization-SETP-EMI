...
%         toc
        ...
        if processed_num+interval > total_num
            ...
            % using GLR test identification --------------------------------------
            % "1" means pre-flitered, "0" means no-flitering
            % [SHP] = GLRT_SelPoint_pol(Mslc,CalWin,Alpha,0);

            % using Wishart distance measure based on similar scattering properties -----------------
            % [SHP] = W_measure(Mslc,CalWin,)
        end
...
%         toc
    ...
    % stamps_write(gamma, 'gamma', 'float32');
    % stamps_write(meancoh, 'meancoh', 'float32');
    % save phase
