function Data=ImgRead_one(imgpath,suffixname,nline,bkformat,machinefmt,Blk,ID)
...
%disp(['Reading', suffixname,' images:' num2str(ID) 'th']);

for ii=1
   % tic;
    if isempty(Blk)
        Data.datastack(:,:,ii)=single(freadbkj([imgpath,tag_files(ID).name],nline,bkformat,machinefmt));
    else
        Data.datastack(:,:,ii)=single(freadbkj([imgpath,tag_files(ID).name],nline,bkformat,machinefmt,Blk(1),Blk(2),Blk(3),Blk(4)));
    end
    temp=regexp(tag_files(ID).name,'\d+','match');
